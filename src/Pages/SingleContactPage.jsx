...
import { api, useGetContactsQuery, useGetSingleContactQuery } from '../services/api'
...
// const data = contactsJson
  ...
  if (data) {
    return (
    <div className={`flex flex-col gap-2 mx-auto max-w-md bg-[#47249e] max-h-fit mt-auto p-5 rounded-xl`}>
        <div className='flex items-center gap-3'>
                <img className={`h-20 w-20 flex-none rounded-full bg-gray-50 border-2 border-black`} src={person.avatar_url} alt="" />
      
            <div>
                <h2 className='text-xl font-bold'>{person.fields['first name'][0]['value']} {person.fields['last name'][0]['value']}</h2>
                <a href={`mailto:${person.fields.email[0].value}`} className='m-0 truncate text-lg hover:text-[#9877f5] w-fit'>
